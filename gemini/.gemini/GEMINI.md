...
When looking for code examples, API usage, library documentation, SDK patterns, or debugging help from the web, prefer the Exa MCP tool (`get_code_context_exa`) over general web search:

Use general web search for non-code research (articles, comparisons, general information) or when Exa results are insufficient.
